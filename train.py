...
def get_args():
    ...
    parser.add_argument('--save_step', type=int, default=30, help='Save every save_step epochs')
...
def main():
    ...
    # retinaface.load_state_dict(torch.load("/versa/elvishelvis/96RetinaYang/out/model_epoch_121.pt"))
    lr=base_lr
    ...
    #             )
    ####
    optimizer = optim.Adam(retinaface.parameters(), lr=lr, weight_decay=1e-6)
    ...
    for epoch in range(args.epochs):
        ...
        for iter_num,data in enumerate(dataloader_train):
            ...
            loss = classification_loss + 0.1*bbox_regression_loss + 0.05*ldm_regression_loss
            loss*=weight
            loss.backward()
            optimizer.step()
